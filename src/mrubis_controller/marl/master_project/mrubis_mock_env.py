...
from marl.master_project.data.data_generator import DataGenerator
from marl.master_project.mrubis_data_helper import get_failing_component, get_current_utility
...
class MrubisMockEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.stats = {}
        self.terminated = False
        self.observation, self.failing_components = self._get_observation(
            self.t)
        self.prior_utility = get_current_utility(self.observation)
        self.action_space = [components for shops,
                             components in self.observation.items()][0].keys()
    ...
    def _get_reward(self, observation):
        ...
        diff_utility = {
            shop: current_utility[shop] - utility for shop, utility in self.prior_utility.items()}
