...
def build_count_plot(base_dir, count_data, episode, shop_distribution):
    ...
    for index, agent in enumerate(shop_distribution):
        ...
        for i in range(episode):
            filter_counts = list(
                (filter(lambda x: x != -1, [count_data[shop][i] for shop in agent])))
            avg = - \
                1 if len(filter_counts) == 0 else sum(
                    filter_counts) / len(filter_counts)
            avgs.append(avg)
        data['Agent ' + str(index)] = avgs
    build_plot(data, f"Tries to find correct issue",
               f"{base_dir}/tries", "Number of Tries")
        ...
        for index, agent in enumerate(shop_distribution):
            ...
            for i in range(episode):
                counts = [regret_data[i][index][shop]
                          for shop in regret_data[i][index]]
                if len(counts) != 0:
...
def write_data(count, data, title, base_dir):
    with open(f"{base_dir}/{title}.txt", "w+") as file:
        for i in range(count):
            data_string = ''.join(
                (str(data[shop][i]) if data[shop][i] > 0 else '') + ',' for shop in data)
...
if __name__ == "__main__":
    build_plot_from_file(
        ['mrubis #1'], '2022_02_04_13_33/tries', 'robustness_shop')
