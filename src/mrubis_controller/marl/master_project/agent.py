...
class Agent:
    ...
    def learn(self, states, actions, reward, states_, dones):
        ...
        for shop_name, action in actions.items():
            ...
            metrics.append({"actor": float(actor_loss),
                           "critic": critic_history.history['loss'][0]})
        return metrics
    ...
    def _build_network(self):
        ...
        for index, dims in enumerate(self.layer_dims):
            name = 'layer_' + (str(index) if index !=
                               len(self.layer_dims) - 1 else 'last')
            next_dense_layer = Dense(
                dims, activation='relu', name=name)(dense_layer)
            dense_layer = next_dense_layer

        probs = Dense(self.n_actions, activation='softmax',
                      name='probs')(dense_layer)
