...
from marl.master_project.mrubis_data_helper import build_replay_buffer
from marl.master_project.mrubis_data_helper import has_shop_remaining_issues
...
class RobustnessComponent:
    def __init__(self, number_of_agents, config=None):
        if config is None:
            self.config = {
                ...
                'actor_upper_threshold': 0.2,
                # value against which the specified statistic is compared
                'critic_lower_threshold': 0,
                # value against which the specified statistic is compared
                'critic_upper_threshold': 1,
        ...
        self.agents_status = {
            index: 'INSUFFICIENT_DATA' for index in range(number_of_agents)}
        # only copy agents that have been OK
        self.previously_stable_agents = {
            index: False for index in range(number_of_agents)}
        ...
        # stores the ranked shop distribution during calibration mode
        self.calibration_distribution = {}
    ...
    def validate_execution(self, agents, agent_index, observations_):
        ...
        if self._is_agent_in_charge_for_other_calibrating_agent(agent_index):
            ...
            failing_agent_shops = self._get_shops_of_agent_in_charge_for_other_calibrating_agent(
                agent_index)
            for n in range(len(failing_agent_shops['failing_agent_index'])):
                challenged_shops = failing_agent_shops['shops'][n].difference(
                    agents[agent_index].shops)
                fixed_shops = [has_shop_remaining_issues(
                    observations_, shop) for shop in challenged_shops]
                if sum(fixed_shops) < len(challenged_shops)/2:
                    ...
                    if len(self.calibration_distribution[
                        ...
                        if self.previously_stable_agents[failing_agent_shops['failing_agent_index'][n]]:
                            self._copy_agent(
                                agents, failing_agent_shops['failing_agent_index'][n])
                        self.agents_status[failing_agent_shops['failing_agent_index']
                                           [n]] = 'RETRAIN'
    ...
    def analyze(self, agents):
        ...
        for index, agent in enumerate(agents):
            ...
            agent_metric = [
                agent_metric for metric in self.metrics for agent_metric in metric[index]][-eval_periods:]
    ...
    def _init_calibration_distribution(self, failing_agent_index, failing_agent, agents):
        ...
        for index, agent in enumerate(agents):
            if agent != failing_agent:
                ...
                for shop_name, components in relevant_history['observations'].items():
                    probabilities = agent.get_probabilities_for_observations(
                        components)
        ...
        ranked_agents = self._rank_agents_predictions(
            relevant_history['observations'], agents_predictions)
    ...
    def _get_shops_of_agent_in_charge_for_other_calibrating_agent(self, agent_index):
        ...
        for failing_agent_index, distribution in self.calibration_distribution.items():
            if len(distribution.keys()) > 0 and agent_index == list(distribution.keys())[0]:
                calibration_distribution['shops'].append(
                    list(distribution.values())[0]['shops'])
                calibration_distribution['failing_agent_index'].append(
                    failing_agent_index)
    ...
    def _rank_agents_predictions(self, observations, agents_predictions):
        ...
        score_per_agent = {agent_index: 0 for agent_index,
                           _ in agents_predictions.items()}  # init scores
    ...
    def _is_agent_check_plausible(self, agent, relevant_history):
        ...
        actions, _regrets = agent.choose_action(
            relevant_history['observations'])
    ...
    def _copy_agent(self, agents, agent_index):
        ...
        load_model = {
            'start_time': agents[agent_index].start_time, 'episode': 'copy', 'index': agent_index}
