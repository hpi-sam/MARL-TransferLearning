...
class Runner:
    def __init__(self, env: MrubisEnv, agent_controllers: ShopAgentController):
        self.env = env
...
class ReplayBufferRunner:
    def __init__(self, env: MrubisEnv, agent_controllers: Iterable[ShopAgentController]):
        self.env = env
        self.agent_controllers = agent_controllers  # TODO create agent controller
            ...
            for episode in range(episodes):
                ...
                for agent_controller in self.agent_controllers:
                    ...
                    while not terminated:
                        ...
                        bar.text = f"Episode: {episode} Step: {step}"
                        step += 1
                        actions = agent_controller.choose_actions(
                            current_observations)
                        # for action in actions.values():
                        #     print(action.action_tensor.max().item(), action.action_tensor.argmax().item(), end=' ')
                        # print()
                        ...
                        agent_controller.add_to_replaybuffer(
                            actions, reward, next_observations)
                        agent_controller.learn_from_replaybuffer(100)
                        current_observations = next_observations
                        if terminated:
                            for shop, count in env_info['stats'].items():
                                print(f"{shop} fixed after: {count}")
                                agent_controller.reset_sampled_actions()
                                if count != -1:
                                    wandb.log(
                                        {f"{shop}_fixed": count}, step=step)
                            self.env.reset()
                    print(f"episode {episode} done")
