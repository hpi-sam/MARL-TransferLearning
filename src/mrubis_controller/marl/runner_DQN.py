...
class DQNRunner:
    def __init__(self, env: MrubisEnv, agent_controller: DQNShopAgentController):
        self.env = env
        ...
        with ap.alive_bar(title="Running") as bar:
            for episode in range(episodes):
                ...
                while not terminated:
                    ...
                    #    print(action.action_tensor.max().item(),
                    #          action.action_tensor.argmax().item(), end=' ')
                    # print()
                    # exit()
                    ...
                    # print(sendable_actions)
                    reward, next_observations, terminated, env_info = self.env.step(
                        sendable_actions)
                    self.agent_controller.learn(
                        actions, reward, current_observations, next_observations)
                    current_observations = next_observations
                    # if step % 100 == 0:
                    # self.agent_controller.copy_current_model()
                    if terminated:
